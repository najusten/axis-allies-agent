...
- ✅ Rectangular board (even-q offset), landscape default 17×13 (odd, so the objective sits on a true centre hex)
...
- ✅ Save / load / report (`stateio.py`): **Game ▾** saves the game to `saves/` (optionally downloading a copy) and loads saved games, reports or an uploaded file, resuming in the same phase with the same player to act; **🐞 Report** writes the exact position plus the player's note and recent log to `reports/`. Both files are the scenario YAML format (axial coords, every UnitState flag, face-down counters, defensive-fire use, road links, controller phase queue), so `python3 scenario.py reports/<file>` replays a report and adding `actions:`/`expect:` turns it into a regression test. `tests/test_saveload.py` checks the round trip is exact and that a loaded game plays on. In-app playtest checklist at `/static/playtest.html` (linked from Game ▾).
- ✅ Fixed battlefields (`maps/*.yaml`, `maplib.py`): hand-authored maps loosely after real ground — Villers-Bocage (Normandy 1944) and Kasserine Pass (Tunisia 1943) — each bringing the armies and years that fought there. The YAML format is compact (terrain lists, roads as waypoints, streams as a course, hedgerow runs) so more can be added by hand.
- ✅ Historical mode picks a match-up that really happened and a battlefield where it was fought (Battlefield: "Match the armies"), leaving out units that entered service after that campaign.
- ✅ Board presentation: terrain symbols, smooth roads that run off the map, wider stream/hedge sides, a Legend panel (every terrain, hex side and marker with its rules effect), and a pace control (fast / normal / slow / step through, with a caption for each opponent action).
...
(2026-09-25, rules audit against the official Q&A) **units without Antiair could never shoot at Aircraft, and Aircraft could never use Bombs/Rockets 8/Strafe/Speed Boost** (those attacks were only generated in the assault phase, which Aircraft never act in) · hull-mounted cannons used the wrong range bands (short = 0-2 instead of 0-1) · Aircraft made defensive fire against ground movement · line of sight to/from Aircraft was blocked by terrain (but not by smoke, which it should be) · **Heavy Armor turned three hits into a destroyed tank instead of a damaged one** · a Soldier in a Pillbox got the +1 bonus but never a cover roll · Entrenched came back every turn instead of being lost for good · a disrupted or boarded Commander lost abilities he should keep · an undeployed Commander lost his initiative bonus · Strike and Fade ignored damage, High Gear and the road bonus · Overrun was applied after the whole move, letting an overrun Soldier still fire · restricted firing arcs could fire at a target on the spine · a failed movement roll didn't end the unit's assault-phase move · five units carried a phantom ability leaked from the next row of the data file (a Pillbox acted as a Tank Obstacle) · obstacles crashed `is_alive` · (2026-09-20) boarded Soldiers could attack from inside any transport (now only from a Fighting Platform that stays put) · transports carried Artillery/Motorcycles/Cavalry · speed-0 Soldiers couldn't board · disrupted Commanders still granted abilities · High Gear moves lost the free first road hex · Superior Camouflage wasn't enforced by the generator · **Vehicles with a subtype ("Vehicle Tank" — i.e. almost all of them) were destroyed by two hits instead of damaged, made cover rolls at 4+ instead of 5+, were skipped by area attacks (rocket salvo) and got no starting facing in fixed setups** (`unit_type == 'Vehicle'` exact matches in casualty/dice/executor/setup) · dismounting could overstack a hex · a unit disrupted by defensive fire / Ace reactions / scenario setup never recovered — recovery only looked at counters flipped in a casualty phase; now every face-up Disrupted counter recovers at the next casualty phase (sticky ones excepted) · two Aircraft could share a hex · spotter bonus for Aircraft crashed on use ·
